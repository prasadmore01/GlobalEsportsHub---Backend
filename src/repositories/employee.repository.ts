...
class EmployeeRepositoryClass extends BaseRepository<Employee> {
    constructor() {
    ...
    /**
     * Find user by UUID with deleted flag
     */
    async findByUuidWithDeleted(uuid: string): Promise<Employee | null> {
        return await this.findOne({ uuid, is_deleted: true } as any);
    }
     ...
     * Change user status
     */
    async changeStatus(id: number, status: string): Promise<Employee | null> {
     ...
     */
    async getEmployeesWithPagination(options: {
        page?: number;
        limit?: number;
        search?: string;
        status?: string;
        ...
        return await this.findAllWithPagination(
            {
                ...
                limit,
                search,
                searchFields: ['first_name', 'last_name', 'email', 'whatsapp_number'],
                sortBy: 'created_at',
                sortOrder: 'DESC'
            },
            additionalWhere
